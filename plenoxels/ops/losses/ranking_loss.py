...
class EdgeguidedRankingLoss(nn.Module):
    ...
    def forward(self, inputs, targets, masks=None):
        ...
        for i in range(n):
            ...
            inputs_A, inputs_B, targets_A, targets_B, masks_A, masks_B = randomSampling(inputs[i,:], targets[i, :], masks[i, :], self.mask_value, random_sample_num)
            # Combine EGS + RS
...
class RenderedRankingLoss(nn.Module):
    def __init__(self, point_pairs=10000, sigma=0.03, alpha=1.0, mask_value=1e-8):
        super(RenderedRankingLoss, self).__init__()
        self.point_pairs = point_pairs # number of point pairs
        self.sigma = sigma # used for determining the ordinal relationship between a selected pair
        self.alpha = alpha # used for balancing the effect of = and (<,>)
        self.mask_value = mask_value
    ...
    def forward(self, inputs, targets, random_sample_num=64, masks=None):
        if masks == None:
            masks = targets > self.mask_value

        #=============================
        n_points = targets.size()
        
        inputs = inputs.view(n_points).double()
        targets = targets.view(n_points).double()
        masks = masks.view(n_points).double()

        # initialization
        loss = torch.DoubleTensor([0.0]).cuda()

        # random_sample_num = 64
        inputs_A, inputs_B, targets_A, targets_B, masks_A, masks_B = randomSampling(inputs, targets, masks, self.mask_value, random_sample_num)
        #GT ordinal relationship
        target_ratio = torch.div(targets_A+1e-6, targets_B+1e-6)
        mask_eq = target_ratio.lt(1.0 + self.sigma) * target_ratio.gt(1.0/(1.0+self.sigma))
        labels = torch.zeros_like(target_ratio)
...
if __name__ == '__main__':
    ...
    rank_loss = RenderedRankingLoss()
    pred_depth = np.random.rand(64,1)
    # pred_depth = np.ones_like(pred_depth)
    # gt_depth = np.random.rand(64,1)
    gt_depth = pred_depth*2 + 10
