...
from plenoxels.datasets import ScannetDataset
...
from plenoxels.utils.warp import Warper
from plenoxels.utils.warp2 import inverse_warp
...
class StaticTrainer(BaseTrainer):
    def __init__(self,
        ...
        self.warper = Warper(resolution=None, device=device)
        super().__init__(
    ...
    def eval_step(self, data, **kwargs) -> MutableMapping[str, torch.Tensor]:
        ...
        channels = {"rgb", "depth", "proposal_depth","entropy"}
    ...
    def validate(self):
        ...
        import numpy as np
        rms = np.zeros(len(dataset), np.float32)
        log_rms = np.zeros(len(dataset), np.float32)
        abs_rel = np.zeros(len(dataset), np.float32)
        sq_rel = np.zeros(len(dataset), np.float32)
        d1 = np.zeros(len(dataset), np.float32)
        d2 = np.zeros(len(dataset), np.float32)
        d3 = np.zeros(len(dataset), np.float32)
        ...
        for img_idx, data in enumerate(dataset):
            ...
            if "scenes" in dataset.datadir:
                out_metrics, _, _,depth_error = self.evaluate_metrics(
                    data["imgs"], ts_render, dset=dataset, img_idx=img_idx,
                    name=None, save_outputs=self.save_outputs)
                
                _,_,abs_rel[img_idx], sq_rel[img_idx], rms[img_idx], log_rms[img_idx], d1[img_idx], d2[img_idx], d3[img_idx] = depth_error
...
def init_tr_data(data_downsample: float, data_dirs: Sequence[str], **kwargs):
    ...
    if dset_type == "scannet":
        max_tr_frames = parse_optint(kwargs.get('max_tr_frames'))
        dset = ScannetDataset(
            data_dir, split='train', downsample=data_downsample,
            max_frames=max_tr_frames, batch_size=batch_size,
            near=kwargs['near'], far=kwargs['far'], fewshot=kwargs['fewshot'],
            patch_rendering=kwargs['patch_rendering'], bbox_radius=kwargs['radius'],
            novel_patch_size = kwargs['novel_patch_size'],stride = kwargs['stride'],novel_depth_loss = kwargs['novel_depth_loss'],
...
def init_ts_data(data_dirs: Sequence[str], split: str, data_downsample=1.0, **kwargs):
    assert len(data_dirs) == 1
    data_dir = data_dirs[0]
    dset_type = decide_dset_type(data_dir)
    ...
    if dset_type == "scannet":
        max_tr_frames = parse_optint(kwargs.get('max_tr_frames'))
        dset = ScannetDataset(
            data_dir, split=split, max_frames=max_tr_frames,
            near=kwargs['near'], far=kwargs['far'], fewshot=None,
            bbox_radius=kwargs['radius'])
...
def load_data(data_downsample, data_dirs, validate_only, render_only, **kwargs):
    ...
    od.update(init_ts_data(data_dirs, split=test_split, data_downsample=data_downsample, **kwargs))
    return od
